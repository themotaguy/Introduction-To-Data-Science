...
def answer_three():
    ...
    diff = (abs(gold['Gold'] - gold['Gold.1'])/gold['Gold.2'])
    return diff.idxmax()
answer_three()
...
def answer_four():
    Points = 3*df['Gold.2'] + 3*df['Silver.2'] + 1*df['Bronze.2']
    return Points
answer_four()
...
def answer_five():
    ...
    topCount = counties.groupby('STNAME').count()['SUMLEV']
    ans = topCount.idxmax()
    return ans
answer_five()
...
def answer_six():
    counties = census_df[census_df['SUMLEV'] == 50]
    popCount = counties.sort_values(by=['STNAME','CENSUS2010POP'], ascending = False).groupby('STNAME').head(3)
    ans = popCount.groupby('STNAME').sum().sort_values(by='CENSUS2010POP', ascending = False).head(3).index.tolist()
...
def answer_seven():
    ...
    maxCounty = max(counties['POP_CHANGE'])
    ans = counties['CTYNAME'][counties['POP_CHANGE'] == maxCounty].tolist()
